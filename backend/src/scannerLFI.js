...
exports.LFIScan = async (url, parameter, reqType, dataType) => {
    ...
    for(let i = 0; i < 10; i++) {
        let output = await fetch(url + encodeURIComponent(`?${parameter}=`) + "../" * i + "etc/passwd", options);
        ...
        if(output.includes('root:x:0:0:')) {
            return [{
                ...
                "payload": url + encodeURIComponent(`?${parameter}=`) + "../" * i + "etc/passwd",
                "recommendations": "Sanitize filepaths and avoid user-inputted filepaths."
            }]
        }
    }
    return [];
